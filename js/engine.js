...
const GameEngine = {
    ...
    render(time) {
        ...
        // Tall grass overlay on all entities (draw grass tips on top of lower body)
        const tgTheme = map.theme && map.theme.tallGrass ? map.theme.tallGrass : { blade: '#4A8530' };
        entities.forEach(entity => {
            const ex = Math.round(entity.type === 'player' ? state.playerX : (entity.npcData ? entity.npcData.x : entity.x / TILE_SIZE));
            const ey = Math.round(entity.type === 'player' ? state.playerY : (entity.npcData ? entity.npcData.y : entity.y / TILE_SIZE));
            const eTile = map.tiles[ey] && map.tiles[ey][ex];
            if (eTile === TILE.TALL_GRASS) {
                ctx.fillStyle = tgTheme.blade;
                const ox = entity.x;
                const oy = entity.y;
                for (let i = 0; i < 4; i++) {
                    const gx = ox + 4 + i * 7;
                    const sway = Math.sin(frame * 0.002 + i + ex) * 2;
                    ctx.beginPath();
